...
if [ $# -ne 3 ]; then
  ...
  echo "Produces ctm_prons in: exp/tri3a_ali"
  exit 1;
fi
...
if [ $stage -le 1 ]; then
  if [ -f $data/segments ] && $use_segments; then
    ...
    [ ! -f $f ] && echo "$0: expecting file $f to exist" && exit 1;
    for n in `seq $nj`; do gunzip -c $dir/ctm_prons.$n.gz; done | \
      ./utils/convert_ctm_prons.pl $data/segments $data/reco2file_and_channel > $dir/ctm_prons || exit 1;
  else
    for n in `seq $nj`; do gunzip -c $dir/ctm_prons.$n.gz; done > $dir/ctm_prons || exit 1;
  fi
  rm $dir/ctm_prons.*.gz
fi
