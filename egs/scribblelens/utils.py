...
def processTranscriptLine(line_, alphabet_, mode_ = 2, buildAlphabetMode_= True, verbose_=0):
    ...
    utils.log("processTranscriptLine() alphabet = " + str(alphabet_.chars) +
              " mode_= " + str(mode_) +
              " buildAlphabetMode= " + str(buildAlphabetMode_), level=logging.DEBUG)
            ...
            if c != '.' and c!= '\'': # Skip these noisy ones
                if not alphabet_.existDict(c):
                    if buildAlphabetMode_:
                        ...
                        utils.log("processTranscriptLine() the symbol :" + str(c) +
